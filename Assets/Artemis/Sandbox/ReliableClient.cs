...
public class ReliableClient : ObjectClient
{
    ...
    private readonly MessageSequenceStorage _outgoingSequenceStorage = new();
    private readonly MessageSequenceStorage _incomingSequenceStorage = new();
    ...
    private void HandleMessage(Message message, Address sender)
    {
        ...
        {
            Debug.LogWarning(
                $"Discarding reliable message #{message.Sequence} as expected sequence is #{expectedSequence}");
            return;
        }

        if (message.DeliveryMethod == DeliveryMethod.Reliable)
        {
            SendObject(new Acknowledgement {Sequence = message.Sequence}, sender);
        }

        Debug.Log($"Received message #{message.Sequence}");
        ...
        {
            case rUDP.Sandbox.Request request:
                HandleRequest(request, sender);
                break;
            case rUDP.Sandbox.Response response:
                HandleResponse(response, sender);
                break;
            default: // Then its a packet with user payload
                HandlePayload(message.Payload, sender);
                break;
        }
    }
    ...
    private void HandleAcknowledgement(Acknowledgement ack, Address sender)
    {
        lock (_pendingAckMessages)
        {
            _pendingAckMessages.Remove(pam => pam.Message.Sequence == ack.Sequence && pam.Recepient == sender);
        }
    }
